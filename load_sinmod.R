...
current$u = ncvar_get(nc, "u_east", start = c(1,1,1,210), count = c(-1,-1,1,1))
current$v = ncvar_get(nc, "v_north", start = c(1,1,1,210), count = c(-1,-1,1,1))
phi = -(pi/180)*(58-current$x)
U = matrix(0, nrow = 211, ncol = 201)
V = matrix(0, nrow = 211, ncol = 201)
for (i in 1:(211*201)){
 rotMat = matrix(c(cos(phi[i]), sin(phi[i]), -sin(phi[i]), cos(phi[i])), nrow = 2)
 rotated = rotMat%*%c(current$u[i], current$v[i])
 U[i] = rotated[1]
 V[i] = rotated[2]
}
#U = current$u
...
#image.plot(U, x = rev(current$x[1,]), y = current$y[,1])
#image.plot(V, x = rev(current$x[1,]), y = current$y[,1])

X = matrix(0,nrow=211, ncol=201)
Y = matrix(0, nrow=211, ncol=201)
vinkel = 58*pi/180
rotMat = matrix(c(cos(vinkel), sin(vinkel), -sin(vinkel), cos(vinkel)), nrow = 2)
for (i in 1:(211*201)){
  rotated = rotMat%*%c(current$x[i], current$y[i])
  X[i] = rotated[1]
  Y[i] = rotated[2]
}
#image.plot(X)
...
current_2$u = ncvar_get(nc_2, "u_east", start = c(1,1,1,210), count = c(-1,-1,1,1))
current_2$v = ncvar_get(nc_2, "v_north", start = c(1,1,1,210), count = c(-1,-1,1,1))
...
image.plot(current$r)
image.plot(current$theta)
...
ggplot(data=df) + geom_segment(aes(x=x, y=y, xend=x+dx, yend=y+dy), arrow = arrow(length = unit(0.08,"cm"))) + theme_bw(base_size=20) + xlab("Longitude (°E)") + ylab("Latitude (°N)")# + coord_cartesian(xlim = c(-3-1.875,12+1.875), ylim = c(65.25,72.75))# + coord_fixed(ratio=1) 
...
ggplot(data=df_2, aes(x=x, y=y)) + geom_segment(aes(xend=x+dx, yend=y+dy), arrow = arrow(length = unit(0.08,"cm"))) + theme_bw(base_size=20) + xlab("Longitude (°E)") + ylab("Latitude (°N)")# + coord_cartesian(xlim = c(-3-1.875,12+1.875), ylim = c(65.25,72.75))# + coord_fixed(ratio=1) 
...
par_est = read.table("C://Users//henri//Documents//GitHub//Master-Thesis//Data//parameter_estimates_single_5_htlp.csv")[,1]
