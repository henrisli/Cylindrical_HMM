...
neg_likelihood_exact_real_htlp_single <- function(parameters_input, n_rows, data_sample, data_sample_2, data_sample_3, data_sample_4, n_cols){
  return(neg_likelihood_exact_htlp(parameters_input, n_rows, data_sample, n_cols) + neg_likelihood_exact_htlp(parameters_input, n_rows, data_sample_2, n_cols) + neg_likelihood_exact_htlp(parameters_input, n_rows, data_sample_3, n_cols) + neg_likelihood_exact_htlp(parameters_input, n_rows, data_sample_4, n_cols))
}

neg_likelihood_exact_hess_real_htlp_single <- function(parameters_input, n_rows, data_sample, data_sample_2, data_sample_3, data_sample_4, n_cols){
  return(neg_likelihood_exact_hess_htlp(parameters_input, n_rows, data_sample, n_cols) + neg_likelihood_exact_hess_htlp(parameters_input, n_rows, data_sample_2, n_cols) + neg_likelihood_exact_hess_htlp(parameters_input, n_rows, data_sample_3, n_cols) + neg_likelihood_exact_hess_htlp(parameters_input, n_rows, data_sample_4, n_cols))
}
...
theta_function_real_htlp_single = function(xi_probs_i_est_and_sample_1, xi_probs_i_est_and_sample_2, xi_probs_i_est_and_sample_3, xi_probs_i_est_and_sample_4, theta_val){
  return(theta_function_htlp(xi_probs_i_est_and_sample_1, theta_val) + theta_function_htlp(xi_probs_i_est_and_sample_2, theta_val) + theta_function_htlp(xi_probs_i_est_and_sample_3, theta_val) + theta_function_htlp(xi_probs_i_est_and_sample_4, theta_val))
}

theta_function_converge_htlp = function(xi_probs_i_est_and_sample, theta_val){
  theta_val = matrix(theta_val, nrow = ncolor)
  #value = 0
  #for (i in 1:A){
  #  j = A_list[[i]][1]
...
theta_function_converge_real_htlp_single = function(xi_probs_i_est_and_sample_1, xi_probs_i_est_and_sample_2, xi_probs_i_est_and_sample_3, xi_probs_i_est_and_sample_4, theta_val){
  return(theta_function_converge_htlp(xi_probs_i_est_and_sample_1, theta_val) + theta_function_converge_htlp(xi_probs_i_est_and_sample_2, theta_val) + theta_function_converge_htlp(xi_probs_i_est_and_sample_3, theta_val) + theta_function_converge_htlp(xi_probs_i_est_and_sample_4, theta_val))
}

full_likelihood_htlp = function(parameter, simulated_sample){
  rho_val = parameter[1]
...
full_likelihood_real_htlp_single = function(parameter, simulated_sample_1, simulated_sample_2, simulated_sample_3, simulated_sample_4){
  return(full_likelihood_htlp(parameter, simulated_sample_1) + full_likelihood_htlp(parameter, simulated_sample_2) + full_likelihood_htlp(parameter, simulated_sample_3) + full_likelihood_htlp(parameter, simulated_sample_4))
}
...
for (iter in 1:n_start){
  ...
  theta_list[[iter]][,1] = runif(ncolor,1/3,1/0.9) # 0.3-1.2
  theta_list[[iter]][,2] = runif(ncolor,1/25,1/5) # 0.04-0.2
  theta_list[[iter]][,3] = runif(ncolor,-pi,pi) # -pi - pi
  theta_list[[iter]][,4] = runif(ncolor,0,3) # 0 - 3
  theta_list[[iter]][,5] = runif(ncolor,0.5,0.99999) # 0-0.99
}
...
for (start_point in 1:n_start){
  ...
  while(T){
    ...
    for (i in 1:A){
      for (j in 1:ncolor^2){
        xi_probs[i,j] = potts_prob[j]*dhtlp(theta_est[xi_A[j,1],], as.vector(simulated_sample[A_list[[i]][1],]))*dhtlp(theta_est[xi_A[j,2],], as.vector(simulated_sample[A_list[[i]][2],]))
        xi_probs_2[i,j] = potts_prob[j]*dhtlp(theta_est[xi_A[j,1],], as.vector(simulated_sample_2[A_list[[i]][1],]))*dhtlp(theta_est[xi_A[j,2],], as.vector(simulated_sample_2[A_list[[i]][2],]))
        xi_probs_3[i,j] = potts_prob[j]*dhtlp(theta_est[xi_A[j,1],], as.vector(simulated_sample_3[A_list[[i]][1],]))*dhtlp(theta_est[xi_A[j,2],], as.vector(simulated_sample_3[A_list[[i]][2],]))
        xi_probs_4[i,j] = potts_prob[j]*dhtlp(theta_est[xi_A[j,1],], as.vector(simulated_sample_4[A_list[[i]][1],]))*dhtlp(theta_est[xi_A[j,2],], as.vector(simulated_sample_4[A_list[[i]][2],]))
      }
    ...
    theta_est_reparam[,5] = log(theta_est_reparam[,5]/(1-theta_est_reparam[,5]))
    
    opt_theta = tryCatch(
      optim(as.vector(theta_est_reparam), fn = theta_function_real_htlp_single, method = "BFGS", xi_probs_i_est_and_sample_1 = cbind(xi_probs_i,simulated_sample), xi_probs_i_est_and_sample_2 = cbind(xi_probs_i_2,simulated_sample_2), xi_probs_i_est_and_sample_3 = cbind(xi_probs_i_3,simulated_sample_3), xi_probs_i_est_and_sample_4 = cbind(xi_probs_i_4,simulated_sample_4), control = list(reltol = 0.01)),
      error = function(e){ 
        exit = T
      }, finally = {}
    )
    ...
    theta_est_new[,5] = exp(theta_est_new[,5])/(1+exp(theta_est_new[,5]))
    
    theta_list[[start_point]] = theta_est_new
    if(is.na(full_likelihood_real_htlp_single(c(opt_rho$par, opt_theta$par), simulated_sample, simulated_sample_2, simulated_sample_3, simulated_sample_4))){break}
    if(abs(full_likelihood_real_htlp_single(c(opt_rho$par, opt_theta$par), simulated_sample, simulated_sample_2, simulated_sample_3, simulated_sample_4) - composite_likelihood[start_point])/(composite_likelihood[start_point])<0.01){
      composite_likelihood[start_point] = full_likelihood_real_htlp_single(c(opt_rho$par, opt_theta$par), simulated_sample, simulated_sample_2, simulated_sample_3, simulated_sample_4)
      conv_exit = T
      break}else{composite_likelihood[start_point] = full_likelihood_real_htlp_single(c(opt_rho$par, opt_theta$par), simulated_sample, simulated_sample_2, simulated_sample_3, simulated_sample_4)}
...
for (start_point in 1:n_start){
  ...
  while(T){
    ...
    if (theta_function_real_htlp_single(as.vector(theta_est_reparam), xi_probs_i_est_and_sample = cbind(xi_probs_i,simulated_sample), xi_probs_i_est_and_sample_2 = cbind(xi_probs_i_2,simulated_sample_2), xi_probs_i_est_and_sample_3 = cbind(xi_probs_i_3,simulated_sample_3), xi_probs_i_est_and_sample_4 = cbind(xi_probs_i_4,simulated_sample_4))!=1e7){break}
    
    opt_theta = tryCatch(
      optim(as.vector(theta_est_reparam), fn = theta_function_converge_real_htlp_single, method = "BFGS", xi_probs_i_est_and_sample_1 = cbind(xi_probs_i,simulated_sample), xi_probs_i_est_and_sample_2 = cbind(xi_probs_i_2,simulated_sample_2), xi_probs_i_est_and_sample_3 = cbind(xi_probs_i_3,simulated_sample_3), xi_probs_i_est_and_sample_4 = cbind(xi_probs_i_4,simulated_sample_4), control = list(reltol = 0.01)),
      error = function(e){ 
        exit = T
      }, finally = {}
    )
...
# parameters_test_reparam[c(10,11)] = log(parameters_test_reparam[c(10,11)]/(1-parameters_test_reparam[c(10,11)]))
...
parameters_test_reparam[c(18,19,20,21)] = log(parameters_test_reparam[c(18,19,20,21)]/(1-parameters_test_reparam[c(18,19,20,21)]))
...
# parameters_test_reparam[c(22,23,24,25,26)] = log(parameters_test_reparam[c(22,23,24,25,26)]/(1-parameters_test_reparam[c(22,23,24,25,26)]))


init_param = parameters_test_reparam

optimal = optim(init_param, neg_likelihood_exact_real_htlp_single, method = "BFGS", control = list(trace=6, REPORT = 1, reltol = 1e-5), n_rows = n_rows, data_sample = simulated_sample, data_sample_2 = simulated_sample_2, data_sample_3 = simulated_sample_3, data_sample_4 = simulated_sample_4, n_cols = n_cols)

write.table(optimal$par, "C://Users//henri//Documents//GitHub//Master-Thesis//Data//parameter_estimates_single_4_htlp.csv")
...
estimated_param[c(18,19,20,21)] = exp(optimal$par[c(18,19,20,21)])/(1+exp(optimal$par[c(18,19,20,21)]))
...
estimated_probabilities_2 = find_back_probs_htlp(optimal$par, n_rows, simulated_sample_2, n_cols)
estimated_probabilities_3 = find_back_probs_htlp(optimal$par, n_rows, simulated_sample_3, n_cols)
estimated_probabilities_4 = find_back_probs_htlp(optimal$par, n_rows, simulated_sample_4, n_cols)
...
values = apply(vals, MARGIN= 1, FUN = dhtlp, param=estimated_param[1,])
image.plot(x=X_cor, y = y_cor, z = matrix(values,nrow=100))
values = apply(vals, MARGIN= 1, FUN = dhtlp, param=estimated_param[2,])
image.plot(x=X_cor, y = y_cor, z = matrix(values,nrow=100))
values = apply(vals, MARGIN= 1, FUN = dhtlp, param=estimated_param[3,])
values[which(values==Inf)]=0
image.plot(x=X_cor, y = y_cor, z = matrix(values,nrow=100))
values = apply(vals, MARGIN= 1, FUN = dhtlp, param=estimated_param[4,])
values[which(values==Inf)]=0
image.plot(x=X_cor, y = y_cor, z = matrix(values,nrow=100))
values = apply(vals, MARGIN= 1, FUN = dhtlp, param=estimated_param[5,])
values[which(values==Inf)]=0
image.plot(x=X_cor, y = y_cor, z = matrix(values,nrow=100))


par_est = read.table("C://Users//henri//Documents//GitHub//Master-Thesis//Data//parameter_estimates_single_4_htlp.csv")[,1]
full_likelihood_real_htlp_single(par_est, simulated_sample, simulated_sample_2, simulated_sample_3, simulated_sample_4)
neg_likelihood_exact_hess_real_htlp_single(par_est, n_rows, simulated_sample, simulated_sample_2, simulated_sample_3, simulated_sample_4, n_cols)

grad_1 = numDeriv::grad(full_likelihood_real_htlp_single, par_est, simulated_sample_1 = simulated_sample, simulated_sample_2 = simulated_sample_2, simulated_sample_3 = simulated_sample_3, simulated_sample_4 = simulated_sample_4)
hess_1 = numDeriv::hessian(full_likelihood_real_htlp_single, par_est, simulated_sample_1 = simulated_sample, simulated_sample_2 = simulated_sample_2, simulated_sample_3 = simulated_sample_3, simulated_sample_4 = simulated_sample_4)
sum(diag(grad_1%*%t(grad_1)%*%solve(hess_1)))

grad_2 = numDeriv::grad(neg_likelihood_exact_hess_real_htlp_single, par_est, n_rows = n_rows, data_sample = simulated_sample, data_sample_2 = simulated_sample_2, data_sample_3 = simulated_sample_3, data_sample_4 = simulated_sample_4, n_cols = n_cols)
hess_2 = numDeriv::hessian(neg_likelihood_exact_hess_real_htlp_single, par_est, n_rows = n_rows, data_sample = simulated_sample, data_sample_2 = simulated_sample_2, data_sample_3 = simulated_sample_3, data_sample_4 = simulated_sample_4, n_cols = n_cols)
