...
for (rep_num in 21:n_replicates){
  ## Draw random field ##
  ...
  true_field_3[rep_num,1:n_cols] = simulate_backward_probs_1(parameters_test_reparam, n_rows, simulated_sample_3[1:n_cols,], n_cols)
  true_field_4[rep_num,1:n_cols] = simulate_backward_probs_1(parameters_test_reparam, n_rows, simulated_sample_4[1:n_cols,], n_cols)
  for (i in 1:n_grid){
    ...
    true_field_3[rep_num,reverse[((i-1)*n_grid+1):(n_grid*i)]] = simulate_backward_probs_1_seed(parameters_test_reparam, n_rows, simulated_sample_3[reverse[((i-1)*n_cols*n_rows+1):(n_rows*n_cols*i)],], n_cols, x_seed = true_field_3[rep_num,i])
    true_field_4[rep_num,reverse[((i-1)*n_grid+1):(n_grid*i)]] = simulate_backward_probs_1_seed(parameters_test_reparam, n_rows, simulated_sample_4[reverse[((i-1)*n_cols*n_rows+1):(n_rows*n_cols*i)],], n_cols, x_seed = true_field_4[rep_num,i])
  }
  ...
  for(i in 1:(n_grid*n_grid)){
    ...
    u = runif(1)
    simulated_sample_drawn_3$theta[i] = ifelse(u<(1+parameters[k_i,5]*sin(theta_1-parameters[k_i,3]))/2, theta_1, -theta_1)
    simulated_sample_drawn_3$x[i] = rweibull(1, scale = 1/(parameters[k_i,2]*(1-tanh(parameters[k_i,4])*cos(simulated_sample_drawn_3$theta[i]-parameters[k_i,3]))^(1/parameters[k_i,1])), shape = parameters[k_i,1])
  }
  ...
  for(i in 1:(n_grid*n_grid)){
    ...
    u = runif(1)
    simulated_sample_drawn_4$theta[i] = ifelse(u<(1+parameters[k_i,5]*sin(theta_1-parameters[k_i,3]))/2, theta_1, -theta_1)
    simulated_sample_drawn_4$x[i] = rweibull(1, scale = 1/(parameters[k_i,2]*(1-tanh(parameters[k_i,4])*cos(simulated_sample_drawn_4$theta[i]-parameters[k_i,3]))^(1/parameters[k_i,1])), shape = parameters[k_i,1])
  }
  ...
  ttime = proc.time()
  optimal = tryCatch(
    optim(init_param, neg_likelihood_exact_real_single, method = "BFGS", control = list(trace=6, REPORT = 1, reltol = 1e-5), n_rows = n_rows, data_sample = simulated_sample_drawn, data_sample_2 = simulated_sample_drawn_2, data_sample_3 = simulated_sample_drawn_3, data_sample_4 = simulated_sample_drawn_4, n_cols = n_cols),
    error = function(e){ 
      T
    }, finally = {}
  )
  ...
  estimated_probabilities_3 = find_back_probs(optimal$par, n_rows, simulated_sample_drawn_3, n_cols)
  estimated_probabilities_4 = find_back_probs(optimal$par, n_rows, simulated_sample_drawn_4, n_cols)
  ...
  write.table(parameter_estimates_1[1:rep_num,], "C://Users//henri//Documents//GitHub//Master-Thesis//Data//parameter_estimates_single_quantile.csv")
  print(rep_num)
}
...
parameter_estimates_1 = as.matrix(read.table("C://Users//henri//Documents//GitHub//Master-Thesis//Data//parameter_estimates_single_quantile.csv"))
